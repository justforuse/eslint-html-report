...
const styles = _.template(fs.readFileSync(path.join(__dirname, 'helpers/styles.html'), 'utf-8')),
  ...
  ruleTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/details/rule.html'), 'utf-8')
  ),
  resultTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/details/result.html'), 'utf-8')
  ),
  ...
  codeTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/details/code/code.html'), 'utf-8')
  ),
  issueTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/details/code/issue.html'), 'utf-8')
  ),
  ...
  rulesTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/summary/rules.html'), 'utf-8')
  ),
  ...
  filesTemplate = _.template(
    fs.readFileSync(path.join(__dirname, 'templates/summary/files.html'), 'utf-8')
  )

/**
 * Get project information including git branch, commit ID, and current timestamp
...
function getProjectInfo() {
  ...
  try {
    ...
    projectInfo.branch = execSync('git rev-parse --abbrev-ref HEAD', {
      encoding: 'utf-8'
    }).trim()
  } catch (error) {
    // Not a git repository or git not available
    console.warn('Unable to get git branch:', error.message)
  }

  try {
    // Get current commit ID (short version)
    projectInfo.commit = execSync('git rev-parse --short HEAD', {
      encoding: 'utf-8'
    }).trim()
  } catch (error) {
    // Not a git repository or git not available
    console.warn('Unable to get git commit:', error.message)
  }
...
function getRuleLink(ruleId) {
  ...
  if (_.startsWith(ruleId, 'angular')) {
    ...
    ruleId = ruleId.replace('@typescript-eslint/', '')
    ruleLink = `https://typescript-eslint.io/rules/${ruleId}`
  } else if (_.startsWith(ruleId, '@intlify/vue-i18n')) {
    ruleId = ruleId.replace('@intlify/vue-i18n/', '')
    ruleLink = `https://eslint-plugin-vue-i18n.intlify.dev/rules/${ruleId}.html`
  } else if (_.startsWith(ruleId, 'import')) {
    ruleId = ruleId.replace('import/', '')
    ruleLink = `https://github.com/import-js/eslint-plugin-import/blob/main/docs/rules/${ruleId}.md`
  } else if (_.startsWith(ruleId, 'vitest')) {
    ruleId = ruleId.replace('vitest/', '')
    ruleLink = `https://github.com/vitest-dev/eslint-plugin-vitest/blob/main/docs/rules/${ruleId}.md`
  }
  return ruleLink
}
...
function getProblemCodeSnippet(result, startLine, endLine) {
  ...
  for (let i = startLine; i <= endLine; i++) {
    if (i === startLine && i === endLine) {
      ...
      // Multi-line issue, handle start line
      codeSnippet += lines[i]
    } else if (i === endLine) {
      // Handle end line
      codeSnippet += lines[i]
  ...
  return htmlEscape(codeSnippet)
}

function renderRulesDetail(results, currDir) {
  const ruleErrors = {}
  ...
  for (const result of results) {
    ...
    for (const message of result.messages) {
      ...
      if (!ruleErrors[ruleId]) {
        ruleErrors[ruleId] = {}
        ruleErrors[ruleId].count = 0
        ruleErrors[ruleId].files = []
      }
      ruleErrors[ruleId].count = ruleErrors[ruleId].count + 1

      if (!ruleErrors[ruleId].occurs) {
        ruleErrors[ruleId].occurs = []
      }
      const codeSnippet = getProblemCodeSnippet(result, message.line - 3, message.endLine + 3)
      ruleErrors[ruleId].occurs.push({ filePath, codeSnippet, ...message })

      // TODO: file may have same error multiple times
      if (!ruleErrors[ruleId].files.includes(filePath)) {
        ruleErrors[ruleId].files.push(filePath)
      }
      ruleErrors[ruleId] = {
        ...ruleErrors[ruleId],
  ...
  // console.log(JSON.stringify(ruleErrors, null, 2))
  // Convert the object into an array of objects where each object has 'ruleId' and 'files' keys
  const res = Object.entries(ruleErrors).map(([ruleId, detail]) => ({
    ruleId,
    detail
  }))
  return res
    .map((result, index) => {
      let template = ruleTemplate({
        index: 'r-' + index,
        fileId: '',
        filePath: result.ruleId,
        color: severityString(result.detail.severity),
        summary: result.detail.files.length + ' file(s)' + ' - ' + result.detail.count + ' times',
        problemCount: result.detail.count
      })

      const fileList = `<tr class="f-r-${index} hidden">
				<td colspan="5">
					<ul>
						${result.detail.occurs
              .map(occur => {
                return `<li>
                  ${occur.filePath.replace(currDir, '')}  Row: ${occur.line} Column: ${
                  occur.column
                } - ${htmlEscape(occur.message)}
                  <p><pre>${occur.codeSnippet}</pre></p>
                </li>`
              })
              .join('<hr />')}
					</ul>
...
function renderResults(results, currDir) {
  // Filter out files with no problems (success files)
  const filesWithProblems = _.filter(results, result =>
    result.errorCount > 0 || result.warningCount > 0
  )

  return _.map(filesWithProblems, function (result, index) {
...
export function generateTemplate(inputData, isMultiOn) {
  const results = typeof inputData === 'string' ? JSON.parse(inputData) : inputData
  ...
  return pageTemplate({
    ...
    rulesDetail: renderRulesDetail(results, currWorkingDir),
    results: renderResults(results, currWorkingDir),
    projectInfo: getProjectInfo(),
    styles:
      isMultiOn && isOutputDirKnown() ? '<link rel="stylesheet" href="./styles.css">' : styles(),
    scripts:
...
export async function generateReport(inputFile, outputFile) {
  try {
    const jsonContent = fs.readFileSync(inputFile, 'utf-8')
    const results = JSON.parse(jsonContent)
    console.log('parse success ', results.length, 'files')

    const html = generateTemplate(jsonContent)
    fs.writeFileSync(outputFile, html, 'utf-8')

    // Calculate total errors and warnings from all files
    let totalErrors = 0
    let totalWarnings = 0
    let totalIssues = 0
    ...
    results.forEach(result => {
      ...
      totalIssues += (result.errorCount || 0) + (result.warningCount || 0)
    })
