...
const styles = _.template(fs.readFileSync(path.join(__dirname, 'helpers/styles.html'), 'utf-8')),
	...
	ruleTemplate = _.template(fs.readFileSync(path.join(__dirname, 'templates/details/rule.html'), 'utf-8')),
...
function getProjectInfo() {
	...
	try {
		...
		projectInfo.branch = execSync('git rev-parse --abbrev-ref HEAD', { encoding: 'utf-8' }).trim();
	} catch (error) {
	...
	try {
		...
		projectInfo.commit = execSync('git rev-parse --short HEAD', { encoding: 'utf-8' }).trim();
	} catch (error) {
		// Not a git repository or git not available
  ...
  return res
    .map((result, index) => {
      ...
      const fileList = `<tr class="f-r-${index} hidden">
              ...
              .map((occur) => {
                return `<li>
                  ${occur.filePath.replace(currDir, '')}  Row: ${occur.line} Column: ${occur.column} - ${htmlEscape(occur.message)}
...
export async function generateReport(inputFile, outputFile) {
  try {
      ...
      console.log('parse success ', messages.length)
